...
import { QuickContacts } from "@/features/transfers/components/quick-contacts";
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      <div className="space-y-3">
        ...
        <QuickContacts
          ...
          onSelect={(triangleId) =>
            setValue("to_triangle_id", triangleId, { shouldValidate: true })
          }
        />
        {!recipientId ? (
          <>
            ...
            />
            <Input
              placeholder={t.transfer.manualPlaceholder}
              onChange={(event) => {
      ...
      <div className="space-y-2">
        <Label htmlFor="description">{t.transfer.note}</Label>
        <Input
          id="description"
      ...
      <Button
        type="submit"
        className="min-h-12 w-full transition-transform duration-300 hover:-translate-y-0.5 active:scale-[0.98]"
        disabled={
          isPending ||
          !recipientId ||
      ...
      >
        {isPending
          ? t.transfer.sending
          : amount
            ? `${t.transfer.send} ${formatCurrency(Number(amount))}`
            : t.transfer.send}
      </Button>
    </form>
  );
